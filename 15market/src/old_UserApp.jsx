                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    <div className="text-[10px] opacity-40">
                      ${Number(trade.entryPrice).toFixed(2)} GÇó {new Date(trade.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                    </div>
                    <div className="flex items-center gap-2">
                      <button
...
export default function UserApp() {
  ...
  // Rounds chart state GÇö populated by RoundsTerminal via onRoundPhaseChange
  ...
  // Ref that always mirrors sessionBalance GÇö used by async callbacks to avoid stale closures
  ...
  const lockedResults = useRef(new Map()); // tradeId GåÆ { status, settlementPrice }
  // =ƒùæn+Å REMOVED TRADES: IDs of trades that have been fully removed from activeTrades.
  ...
  const reconcileTrades = useCallback((backendAllRaw) => {
    ...
    setActiveTrades(prev => {
      ...
      const backendActive = backendAll.filter(t => {
        ...
        // =ƒùæn+Å Never re-insert trades that have been fully removed from active view
  ...
  // Periodic session balance refresh (every 12s) GÇö catches cases where
  ...
  const executeTrade = async (params = null) => {
    ...
    try {
      ...
      // GöÇGöÇGöÇ ROUNDS P2P (REAL CONTRACT & SESSION SUPPORT) GöÇGöÇGöÇ
      ...
      // GöÇGöÇGöÇ CLASSIC TRADING GÇö EMBEDDED WALLET MODEL GöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇ
      ...
      const backgroundTrade = async () => {
        ...
        try {
          ...
          // G£à Trade Active (On-chain stake moved)
          ...
          notify('Trade Active G£ô', 'success');
        } catch (err) {
          clearTimeout(timeoutId);
          ...
          setActiveTrades(prev => prev.filter(t => t.id !== tradeId));
          setTradeHistory(prev => prev.filter(t => t.id !== tradeId));
          notify(err.name === 'AbortError' ? 'GÜí Network Congested GÇö Trade Cancelled' : err.message, 'error');
        }
      };
  ...
  useEffect(() => {
    ...
    fetchTradeHistory();
    // 30s fallback poll GÇö socket events handle instant updates now
  ...
  // GöÇGöÇGöÇ UNIFIED PRICE CONSUMPTION GöÇGöÇGöÇ
  ...
  // GöÇGöÇGöÇ SOCKET.IO: Trade events + Backend-Authoritative Settlement GöÇGöÇGöÇ
  useEffect(() => {
    ...
    // GöÇGöÇ BACKEND-AUTHORITATIVE SETTLEMENT GöÇGöÇ
  ...
  // GöÇGöÇ Backend-Authoritative Trade Ticks GöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇ
    ...
    finished.forEach(trade => {
      ...
      if (tid && !cleanupTimers.current[tid]) {
        ...
        cleanupTimers.current[tid] = setTimeout(() => {
          // =ƒùæn+Å Mark as permanently removed so the reconciler never re-adds it
  ...
  // Arc Settlement Listener GÇö with dedup to prevent double-crediting
  ...
  const handleRefill = useCallback(async (amt) => {
    ...
    try {
      ...
      if (!walletClient) {
        notify("Wallet client not ready GÇö please reconnect your wallet", "error");
        return;
      }
  ...
  const handleWithdraw = useCallback(async (amt) => {
    ...
    try {
      ...
      try {
        ...
        console.log("G£à [WITHDRAW] User authorized");
      } catch (sigErr) {
      ...
      try {
        ...
        if (fetchErr.name === 'AbortError') {
          throw new Error("Network timeout GÇö Arc RPC may be congested. Try again in a moment.");
        }
        throw fetchErr;
      } finally {
                  ...
                  >
                    ...
                    {!isSmallScreen && (
                      ...
                      <>
                        ...
                        {/* Active Trade / Controls Box GÇö hidden in Rounds */}
                        {gameMode !== 'rounds' && (
            ...
            <div className="flex items-center justify-center gap-3 py-1.5 px-4 overflow-hidden">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[9px] font-black text-white uppercase tracking-[0.3em]">
                  Disconnected GÇó Internet Connection Lost
                </span>
              </div>
            </div>
          </motion.div>
          ...
          >
            <div className="flex items-center justify-center gap-3 py-1.5 px-4">
              <span className="text-[9px] font-black text-white uppercase tracking-[0.3em] flex items-center gap-2">
                <CheckCircle size={10} />
                Network Reconnected GÇó System Online
              </span>
            </div>
          </motion.div>
        )}
