          ...
          >
            ...
            >
              ...
              <div className="absolute inset-0 pointer-events-none select-none z-0 overflow-hidden rounded-3xl opacity-[0.45]">
                ...
                >
                  <defs>
                    {/* Very faint, ethereal cyber glow aura */}
                    ...
                    {/* 
                      Dual-Axis Fade Mask:
                      - 100% plain obsidian behind the 15market logo (x: 0 to 145px)
                      - Smooth dissolve downwards (y: 110px to 210px) keeping bottom trade stats pure and crisp
                    */}
                  ...
                  <g mask="url(#deepFadedSciFiMask)">
                    {/* 1. Soft Sub-Surface Atmospheric Caustic Glow */}
                    <ellipse cx="270" cy="50" rx="230" ry="110" fill="url(#fadedCyberAura)" />

                    {/* 2. Very Faded Isometric Cyber Grid Floor */}
                    <g opacity="0.07">
                      {[15, 35, 60, 90, 125, 160].map((y, idx) => (
                        <line
                          key={`faded-hgrid-${idx}`}
                          x1="130" y1={y} x2="490" y2={y}
                          stroke={accent}
                          strokeWidth="0.5"
                        />
                      ))}
                    ...
                    {/* 3. Sub-surface Circuit Highway Traces */}
                    <g opacity="0.35">
                      <path
                        d="M 140 25 L 195 25 L 225 55 L 350 55 L 380 25 L 475 25"
                        stroke="url(#fadedTraceGrad)"
                        strokeWidth="1"
                        fill="none"
                      />
                      <path
                        d="M 160 70 L 210 70 L 230 90 L 320 90 L 345 65 L 430 65"
                        stroke="url(#fadedTraceGrad)"
                        strokeWidth="0.75"
                        strokeDasharray="4 6"
                        fill="none"
                      ...
                      {/* Micro Node Dots */}
                      {[
                        [195, 25], [225, 55], [350, 55], [380, 25],
                        [210, 70], [230, 90], [320, 90], [345, 65]
                      ].map(([nx, ny], idx) => (
                        <circle key={`faded-node-${idx}`} cx={nx} cy={ny} r="1.5" fill={accent} opacity="0.6" />
                      ))}
                    </g>

                    {/* 4. Minimalist HUD Reticle & Crosshair (Soft & Faded) */}
                    <g opacity="0.18">
                      ...
                      {[0, 90, 180, 270].map((deg) => (
                        <line
                          key={`faded-cardinal-${deg}`}
                          x1={245 + 63 * Math.cos((deg * Math.PI) / 180)}
                          y1={60 + 63 * Math.sin((deg * Math.PI) / 180)}
                          x2={245 + 69 * Math.cos((deg * Math.PI) / 180)}
                          y2={60 + 69 * Math.sin((deg * Math.PI) / 180)}
                          stroke={accent}
                          strokeWidth="1"
                        />
                      ))}
                    ...
                    {/* 5. Minimal Sci-Fi Telemetry Labels (Very Faint Monospace) */}
              ...
              <div className="relative z-10 flex items-stretch gap-4 px-4 pt-11 pb-2 h-[calc(100%-20px)]">
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-between">
                  ...
                  {/* Amount + Details Row - shifted upwards by 10% */}
                  <div className="mt-[-10%]">
                    {/* Big Amount */}
                    <div className="my-1">
                      <div
              ...
              >
                ...
                {/* Won / Lose Label at top of banner */}
              ...
              {/* QR Code with outline - centered on the banner */}
              <div
              ...
              >
                ...
                <div className="mt-2 text-center w-full">
                  <div className="text-white/80 text-[7px] font-bold tracking-widest uppercase" style={{ fontFamily: '"Comfortaa", cursive' }}>
                    Trade ID
                  </div>
                  <div className="flex flex-col items-center gap-0.5 mt-1 w-full">
                    {(() => {
                      ...
                      return chunks.map((chunk, i) => (
                        <div
                          key={i}
                          className="text-white text-[13px] font-black tracking-[0.16em] leading-tight"
                          style={{ fontFamily: '"Comfortaa", cursive' }}
                        >
                          {chunk}
                        </div>
