    ...
    return (
        <div className="flex flex-col gap-3 relative min-h-0 h-full">
            ...
            <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar space-y-3 min-h-0">
                {activeTrades.length > 0 ? (
                    (() => {
                        const visibleTrade = activeTrades[0];
                        const othersCount = activeTrades.length - 1;

                        return (
                            <>
                                <div key={visibleTrade.id}>
                                    {/* Render the most recent trade */}
                                    {(() => {
                                ...
                                {othersCount > 0 && (
                                    <div className={`mt-2 py-2 px-3 rounded-lg border flex items-center justify-center gap-2 ${isLight ? 'bg-black/5 border-black/10' : 'bg-white/5 border-white/10'}`}>
                                        <div className="w-1.5 h-1.5 rounded-full bg-[#3CB371] animate-pulse" />
                                        <span className={`text-[8px] font-black uppercase tracking-widest ${isLight ? 'text-black/50' : 'text-white/40'}`}>
                                            +{othersCount} more active trade{othersCount > 1 ? 's' : ''} running
                                        </span>
                                    </div>
                                )}
                            </>
