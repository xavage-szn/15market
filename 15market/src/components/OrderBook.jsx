    ...
    return (
        <div className={`w-full h-full flex flex-col font-mono text-[10px] lg:text-xs overflow-hidden rounded-xl transition-colors duration-300 ${isDark ? 'text-white/80' : 'text-[#0a261a]'}`}>
            ...
            <div className="flex-1 overflow-hidden flex flex-col justify-end gap-[1px]">
                {asks.map((ask, i) => (
                    <div key={i} className={`flex items-center justify-between px-3 py-0.5 relative group cursor-pointer ${isDark ? 'hover:bg-white/5' : 'hover:bg-[#3CB371]/5'} transition-colors`}>
                        <div
