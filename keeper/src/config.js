...
if (!process.env.PROGRAM_ID) {
    console.error("❌ ERROR: PROGRAM_ID missing!");
    process.exit(1);
}

const PROGRAM_ID = new PublicKey(process.env.PROGRAM_ID.trim());
const START_TIME = Math.floor(Date.now() / 1000) - 86400;
...
if (!keypair) {
    console.error("❌ No keypair found. Exiting.");
    process.exit(1);
}
...
// Load IDL from ../idl/sol_prediction.json
const idlPath = path.resolve(__dirname, "../idl/sol_prediction.json");
