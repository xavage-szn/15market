    ...
    if (pathName === '/health') {
        ...
        if (req.method === 'POST') {
            readBody(data => {
                ...
                // Allow explicit direction if provided
                if (data.direction !== undefined) {
                    newTrade.direction = Number(data.direction) === 1 ? "UP" : "DOWN";
                }
...
async function fetchPendingBets() {
    try {
        ...
        const accounts = await readConnection.getProgramAccounts(program.programId, {
            ...
            if (err.message.includes("getProgramAccounts is not available")) {
                // Silently swallow known RPC restriction on free tiers
                return [];
            }
            throw err;
        });
...
async function fetchProtocolHistory() {
    try {
        const [profiles, bets] = await Promise.all([
            readConnection.getProgramAccounts(program.programId, { filters: [{ dataSize: 104 }] }),
            readConnection.getProgramAccounts(program.programId, {
                filters: [
                    { dataSize: 112 },
        ...
        if (!e.message.includes("getProgramAccounts is not available")) {
...
async function runKeeper() {
    ...
    console.log(`[KEEPER] 📡 Connecting to WebSocket for real-time monitoring...`);

    // Register simple listener for live logs - THIS IS THE PRIMARY SOURCE FOR NEW BETS
    ...
    while (true) {
        try {
            ...
            if (readyToSettle.length > 0) {
                ...
                if (burstMode) {
                    console.log(`[KEEPER] 🔥 BURST MODE ACTIVE: Processing ${uniqueBets.length} pending settlements...`);
                } else {
                    console.log(`[KEEPER] 🔍 Processing ${uniqueBets.length} settlement(s)...`);
                }
                ...
                for (let i = 0; i < uniqueBets.length; i += CONCURRENCY) {
                    ...
                    await Promise.all(chunk.map(async (bet) => {
                        ...
                        const age = now - (bet.expiry - bet.duration);
                        if (age < 5) return;
                        ...
                        const isWinByDirection = (bet.direction === 1) ? (price >= bet.entryPrice) : (price <= bet.entryPrice);
