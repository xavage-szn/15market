...
/**
 * 15MARKET AI ARBITER for Arc Keeper (Multi-Asset)
 */
class AiArbiter {
    constructor() {
    ...
    async confirmPrice(symbol = 'BTC') {
        ...
        this._currentFetches[symbol] = (async () => {
            try {
                ...
                const validPrices = results.filter(r => r.status === 'fulfilled' && r.value > 0).map(r => r.value);
                ...
                let consensus;
                if (validPrices.length >= 4) consensus = validPrices.slice(1, -1).reduce((a, b) => a + b, 0) / (validPrices.length - 2);
                else consensus = validPrices.reduce((a, b) => a + b, 0) / validPrices.length;
    ...
    async getResultVerdict(entryPrice, symbol = 'BTC') {
        ...
        if (cache && (now - cache.time < 1000)) {
            settlePrice = cache.price;
        } else {
            settlePrice = await this.confirmPrice(symbol);
        }

        if (!settlePrice || settlePrice <= 0) return { price: 0, isReliable: false };

        console.log(`🧠 [AI_ARBITER] ${symbol} Result: $${settlePrice.toFixed(4)} (Entry: $${entryPrice.toFixed(4)})`);
