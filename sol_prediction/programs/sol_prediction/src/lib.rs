...
declare_id!("9bK6W6uAS2xC9Xdw6fcMgHH1BwLp4gJDKj5dwciF8CPe");
...
pub mod sol_prediction {
    ...
    pub fn place_bet(ctx: Context<PlaceBet>, direction: u8, amount_lamports: u64, entry_price: u64, nonce: u64, duration: u8) -> Result<()> {
        ...
        // Send stake directly to treasury
        ...
        // Use main_owner from remaining accounts if provided, otherwise default to signer
        bet.main_owner = ctx.remaining_accounts.get(0)
            .map(|a| a.key())
            .unwrap_or(ctx.accounts.user.key());
    ...
    pub fn settle_bet(ctx: Context<SettleBet>, user_won: bool) -> Result<()> {
        ...
        let main_owner = bet.main_owner;
        
        // Multiplier: 5s=6.98x, 10s=4.98x, 15s=1.98x
        ...
        // Update profile if provided (matches main_owner)
...
pub struct PlaceBet<'info> {
    #[account(
        ...
        payer = user, 
        space = 8 + Bet::LEN, 
        seeds = [b"bet_v7", user.key().as_ref(), nonce.to_le_bytes().as_ref()], 
        bump
    )]
...
pub struct SettleBet<'info> {
    #[account(
        mut, 
        close = owner, 
        seeds = [b"bet_v7", bet.owner.as_ref(), bet.nonce.to_le_bytes().as_ref()], 
        bump = bet.bump
    )]
    pub bet: Account<'info, Bet>,
    #[account(mut)]
...
impl Bet { pub const LEN: usize = 32 + 32 + 8 + 1 + 8 + 8 + 1 + 8 + 1 + 1; }
