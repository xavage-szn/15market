...
const MessagingSystem = ({ wallet, connection, isOpen, onClose, isAdminView = false, adminRole = null, embedded = false }) => {
                        ...
                        <div key={i} className={`flex flex-col ${m.sender === myAddress ? 'items-end' : 'items-start'}`}>
                            ...
                            <div className="flex items-center gap-1 mt-1.5 px-1">
                                <span className={`text-[7px] uppercase font-black tracking-widest ${getRoleColor(m.sender)}`}>
                                    {formatAddress(m.sender)} • {new Date(m.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                </span>
