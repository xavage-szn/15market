      ...
      try {
        ...
        if (sessionMode && sessionBalance >= (Number(amount) + 0.005)) {
          ...
          try {
            ...
            const contract = new ethers.Contract(ARC_CONTRACT_ADDRESS, ArcABI.abi, evmSessionWallet);
          ...
          try {
            const hash = await writeContractAsync({
              ...
              args: [Number(dirVal), BigInt(duration), BigInt(entryPriceParams), Number(assetId)],
              value: amountWei,
        ...
        const newTrade = {
          id: nonceValue, direction, amount: Number(amount).toFixed(4), entryPrice: activePrice.toFixed(4),
          timestamp: new Date().toLocaleTimeString(), status: "PENDING", tx: txHash, nonce: nonceValue,
    ...
    for (const commitment of commitments) {
      ...
      while (retries > 0) {
        try {
          ...
          console.warn(`Retrying blockhash fetch (${commitment})...`, e);
          retries--;
          await new Promise(r => setTimeout(r, 800));
        }
      }
    }
    throw new Error("Failed to fetch a recent blockhash after multiple attempts across commitments.");
  };
      ...
      <div className="w-full max-w-7xl mb-6 flex flex-col gap-4">
        {winnerBanner && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className={`w-full ${theme === 'light' ? 'static-panel-light' : 'static-panel'} !rounded-2xl mb-6 p-4 lg:p-6 border !border-white/5 relative`}
          >
        ...
        {campaigns.filter(c => Date.now() < c.endTime && (c.network === 'general' || c.network === network)).map(camp => (
          <motion.div
            ...
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className={`w-full ${theme === 'light' ? 'static-panel-light' : 'static-panel'} !rounded-2xl p-6 mb-4 flex flex-col md:flex-row items-center justify-between gap-6 transition-all duration-500`}
          >
