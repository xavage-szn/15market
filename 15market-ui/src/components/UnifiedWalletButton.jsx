...
import { useAppKit, useAppKitAccount, useDisconnect, useAppKitState, useAppKitNetwork } from '@reown/appkit/react';
...
export const UnifiedWalletButton = ({ currentNetwork, onNetworkChange, theme }) => {
    ...
    const handleChainSelect = async (newChainId) => {
        ...
        try {
            ...
            // 4. Switch AppKit Network Context & Prompt New Connection
            setTimeout(async () => {
                try {
                    // Network ID mapping for AppKit
                    const networkIdMap = {
                        'solana': 'solana:EtWTRABZaYq6iMfeYKouRu166VU2xqa1', // Solana Devnet
                        'arc': 'eip155:5042002' // Arc Testnet
                    };

                    // Switch network context before opening modal
                    if (switchNetwork && networkIdMap[newChainId]) {
                        console.log(`🔄 Switching AppKit to ${newChainId} network...`);
                        await switchNetwork(networkIdMap[newChainId]);
                    }
                } catch (switchErr) {
